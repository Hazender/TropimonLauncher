...
Java_com_hazender_tropimonlauncher_bridge_LoggerBridge_start(JNIEnv *env, __attribute((unused)) jclass clazz, jstring logPath) {
    if (latestlog_fd != -1)
    {
    ...
    {
        jclass eventLogListener = (*env)->FindClass(env, "com/hazender/tropimonlauncher/bridge/LoggerBridge$EventLogListener");
...
JNIEXPORT void JNICALL Java_com_hazender_tropimonlauncher_bridge_LoggerBridge_append(JNIEnv *env, __attribute((unused)) jclass clazz, jstring text) {
...
JNIEXPORT void JNICALL
Java_com_hazender_tropimonlauncher_bridge_LoggerBridge_setListener(JNIEnv *env, __attribute((unused)) jclass clazz, jobject log_listener) {
...
JNIEXPORT void JNICALL
Java_com_hazender_tropimonlauncher_bridge_ZLBridge_setupExitMethod(JNIEnv *env, jclass clazz,
                                                        jobject context) {
    exitTrap_ctx = (*env)->NewGlobalRef(env,context);
    (*env)->GetJavaVM(env,&exitTrap_jvm);
    exitTrap_exitClass = (*env)->NewGlobalRef(env,(*env)->FindClass(env,"com/hazender/tropimonlauncher/bridge/ZLNativeInvoker"));
